 ...
 * Never calls API directly. Never writes to State directly.
 */
...
const UI = (() => {
  ...
  const INQ_STATUS_LABEL  = { new:'New', contacted:'Contacted', in_progress:'In Progress', closed:'Closed' };
  const TKT_STATUS_LABEL  = { open:'Open', in_progress:'In Progress', pending:'Pending', resolved:'Resolved', closed:'Closed' };
  const TKT_PRI_LABEL     = { low:'Low', medium:'Medium', high:'High', critical:'Critical' };
  ...
  function renderTabs() {
    ...
    if (inqPane) inqPane.style.display = activeTab === 'inquiries' ? 'flex' : 'none';
    if (tktPane) tktPane.style.display = activeTab === 'tickets'   ? 'flex' : 'none';
  ...
  function renderInquiryList() {
    ...
    // Build fragment for new rows, update existing
    ...
    items.forEach(inq => {
      ...
      row.className = ['inquiry-row', inq.id===activeId?'active':'', !inq.read&&inq.id!==activeId?'unread':'', overdue?'overdue':''].filter(Boolean).join(' ');
      row.innerHTML = `
        ...
        <div class="row-body">
          ...
          <div class="row-chips">
            ...
            <span class="chip chip-muted">${INQ_STATUS_LABEL[inq.status]||inq.status}</span>
          </div>
        </div>`;
      if (isNew) container.appendChild(row);
    });

    // Remove rows that no longer appear
  ...
  function renderInquiryDetail(id) {
    ...
    body.innerHTML = `
      <div class="detail-header">
        ...
        <span class="status-badge status-badge-${inq.status}">${INQ_STATUS_LABEL[inq.status]||inq.status}</span>
      </div>

      <div class="detail-actions">
        <a href="tel:${esc(inq.phone)}" class="btn btn-primary">
          <svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M22 16.92v3a2 2 0 01-2.18 2 19.79 19.79 0 01-8.63-3.07A19.5 19.5 0 014.13 15.8 19.79 19.79 0 011.06 7.16 2 2 0 013.05 5h3a2 2 0 012 1.72c.127.96.361 1.903.7 2.81a2 2 0 01-.45 2.11L7.09 12.91a16 16 0 006 6z"/></svg>
          Call
        </a>
        ...
        <button class="btn btn-secondary" id="inq-mark-contacted" ${inq.status!=='new'?'disabled':''}>Mark Contacted</button>
      </div>
      ...
      <div class="detail-section">
        ...
        <div class="controls-row">
          <label class="control-group">
            ...
            <select id="inq-status-sel">
              ${['new','contacted','in_progress','closed'].map(s=>`<option value="${s}"${inq.status===s?' selected':''}>${INQ_STATUS_LABEL[s]}</option>`).join('')}
            </select>
          </label>
          <label class="control-group">
            <span class="control-label">Assigned to</span>
            <select id="inq-assign-sel">
  ...
  function renderTicketList() {
    ...
    items.forEach(tkt => {
      ...
      row.className = ['ticket-row', tkt.id===activeId?'active':'', !tkt.read&&tkt.id!==activeId?'unread':'', overdue?'overdue':''].filter(Boolean).join(' ');
      row.innerHTML = `
        ...
        <div class="row-body">
          ...
          <div class="row-chips">
            <span class="chip chip-pri-${tkt.priority}">${TKT_PRI_LABEL[tkt.priority]}</span>
            <span class="chip chip-tkt-${tkt.status}">${TKT_STATUS_LABEL[tkt.status]||tkt.status}</span>
            ${overdue ? '<span class="chip chip-danger">Overdue</span>' : ''}
  ...
  function renderTicketDetail(id) {
    ...
    body.innerHTML = `
      <div class="detail-header">
        ...
        <div class="detail-title-block">
          <div style="display:flex;align-items:center;gap:8px;flex-wrap:wrap">
            ...
            <span class="chip chip-pri-${tkt.priority}" style="font-size:11px">${TKT_PRI_LABEL[tkt.priority]}</span>
            ${overdue ? '<span class="chip chip-danger" style="font-size:11px">Overdue</span>' : ''}
          </div>
          <h2 class="detail-name" style="margin-top:4px">${esc(tkt.subject)}</h2>
          <p class="detail-sub">${esc(tkt.client_name)} · ${esc(tkt.project||'')}</p>
        </div>
        <span class="status-badge status-badge-tkt-${tkt.status}">${TKT_STATUS_LABEL[tkt.status]||tkt.status}</span>
      </div>
      ...
      <div class="detail-actions">
        ...
        <button class="btn btn-success"   id="tkt-resolve-btn" ${['resolved','closed'].includes(tkt.status)?'disabled':''}>Mark Resolved</button>
        <button class="btn btn-ghost"     id="tkt-close-btn"   ${tkt.status==='closed'?'disabled':''}>Close Ticket</button>
      </div>
      ...
      <div class="detail-section" style="display:flex;gap:20px;flex-wrap:wrap">
        <div style="flex:1;min-width:180px">
          ...
          <table class="info-table"><tbody>
            <tr><td>Client</td><td>${esc(tkt.client_name)}</td></tr>
        ...
        <div style="flex:1;min-width:180px">
          ...
          <table class="info-table"><tbody>
            ...
            <tr><td>Priority</td><td>${TKT_PRI_LABEL[tkt.priority]}</td></tr>
            <tr><td>Status</td><td>${TKT_STATUS_LABEL[tkt.status]||tkt.status}</td></tr>
          </tbody></table>
        </div>
      </div>
      ...
      <div class="detail-section">
        ...
        <div class="controls-row">
          <label class="control-group">
            <span class="control-label">Status</span>
            <select id="tkt-status-sel">
              ${['open','in_progress','pending','resolved','closed'].map(s=>`<option value="${s}"${tkt.status===s?' selected':''}>${TKT_STATUS_LABEL[s]}</option>`).join('')}
            </select>
          </label>
          <label class="control-group">
            <span class="control-label">Priority</span>
            <select id="tkt-pri-sel">
              ${['low','medium','high','critical'].map(p=>`<option value="${p}"${tkt.priority===p?' selected':''}>${TKT_PRI_LABEL[p]}</option>`).join('')}
            </select>
          </label>
          <label class="control-group">
            <span class="control-label">Assigned to</span>
            <select id="tkt-assign-sel">
      ...
      <div class="detail-section" id="replies-section">
        ...
        <div id="reply-thread">${renderRepliesHTML(tkt.replies||[])}</div>
        <div class="reply-compose">
          <textarea id="reply-input" placeholder="Write a reply to the client..." rows="3"></textarea>
  ...
  function renderRepliesHTML(replies) {
    if (!replies.length) return '<p style="color:var(--text-muted);font-size:13px;padding:8px 0">No replies yet.</p>';
    return replies.map(r => `
      <div class="reply-item reply-${r.author_role}">
        <div class="reply-meta">
          <span class="reply-author">${esc(r.author)}</span>
          <span class="reply-role-chip ${r.author_role==='client'?'chip-info':'chip-muted'}">${r.author_role==='client'?'Client':'Staff'}</span>
  ...
  function appendReply(reply) {
    ...
    if (!thread) return;
    const existing = thread.querySelector('p'); // "no replies" placeholder
    if (existing) thread.innerHTML = '';
    thread.insertAdjacentHTML('beforeend', renderRepliesHTML([reply]));
